...
#check-wan connection status only if /etc/xmproxy/check-wan-periodically file exists
if [ -f "/etc/xmproxy/check-wan-periodically.txt" ]; then
	echo "check-wan is enabled - proceed with checking the wan status" > /dev/null
else
	exit 0
fi
